...
exports.showShops = async (req, res, next) => {
    ...
    if(cat === undefined) // when there is no querry for category , it becomes undefined => in case of no query for category we search every category
        categ = ids ;
    ...
    if(location === undefined) // same undefined case vala reason like category filter
    ...
    const count = await Shop.find({ ...keyword, category: categ, location: locationFilter }).countDocuments();

    try {
        const shops = await Shop.find({ ...keyword, category: categ, location: locationFilter }).sort({ createdAt: -1 }).skip(pageSize * (page - 1)).limit(pageSize)
        res.status(200).json({
            success: true,
            shops,
            page,
